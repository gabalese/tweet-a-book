...
class EPUB(zipfile.ZipFile):
    ...
    @property
    def author(self):
        return self.info["metadata"]["dc:creator"].text

    @author.setter
    def author(self, value):
        tmp = self.opf.find(".//{0}creator".format(NAMESPACES["dc"]))
        tmp.text = value
        self.info["metadata"]["dc:creator"] = value

    @property
    def title(self):
        return self.info["metadata"]["dc:title"].text
    ...
    def title(self, value):
        ...
        self.info["metadata"]["dc:title"] = value

    @property
    def language(self):
        return self.info["metadata"]["dc:language"]

    @language.setter
    def language(self, value):
        tmp = self.opf.find(".//{0}language".format(NAMESPACES["dc"]))
        tmp.text = value
        self.info["metadata"]["dc:language"] = value
